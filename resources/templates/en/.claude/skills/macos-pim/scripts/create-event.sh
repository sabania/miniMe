...
# Usage: bash create-event.sh "Calendar" "Title" "2026-03-01 14:00" "2026-03-01 15:00" [location] [notes]
...
if [ -z "$CALENDAR" ] || [ -z "$TITLE" ] || [ -z "$START" ] || [ -z "$END" ]; then
    echo "Usage: bash $0 \"Calendar\" \"Title\" \"YYYY-MM-DD HH:MM\" \"YYYY-MM-DD HH:MM\" [location] [notes]"
    exit 1
fi
...
tell application "Calendar"
    ...
    return "OK: Event created in $SAFE_CALENDAR — $SAFE_TITLE ($SAFE_START to $SAFE_END)"
end tell
EOF
