#!/bin/bash
# Email senden via Mail.app
# Usage: bash send-email.sh "to@email.com" "Betreff" "Nachricht" [from@email.com]
...
if [ -z "$TO" ] || [ -z "$SUBJECT" ] || [ -z "$BODY" ]; then
    echo "Usage: bash $0 \"to@email.com\" \"Betreff\" \"Nachricht\" [from@email.com]"
    exit 1
fi
...
tell application "Mail"
    ...
    send newMessage
    return "OK: Mail gesendet an $SAFE_TO"
end tell
EOF
